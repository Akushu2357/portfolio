        ...
        <div className="grid lg:grid-cols-2 gap-6 lg:gap-8">
          ...
          <div className="bg-white rounded-xl p-4 sm:p-6 lg:p-8 h-[70vh] flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto scrollbar-hide">
              {/* Timeline Container with proper padding and positioning */}
              <div className="relative pl-8 sm:pl-10 lg:pl-12 pr-2 sm:pr-4 py-6 h-full">
                {/* Vertical Timeline Line - Fixed positioning for all screen sizes */}
                <div className="absolute left-4 sm:left-5 lg:left-6 top-10 bottom-10 w-0.5 bg-gradient-to-b from-cyan-500 to-blue-500 z-0"></div>
                ...
                <div className="space-y-8 sm:space-y-10 lg:space-y-12 relative z-10">
                  {experiences.map((exp, index) => (
                    ...
                    >
                      {/* Timeline Node - Properly positioned for all screen sizes */}
                      <div className={`absolute left-[-23px] sm:left-[-25px] lg:left-[-31px] top-4 sm:top-5 lg:top-6 w-3 h-3 sm:w-3.5 sm:h-3.5 lg:w-4 lg:h-4 bg-white border-3 sm:border-4 rounded-full z-20 transition-all duration-300 ${
                        selectedExperience === index ? 'border-cyan-500 bg-cyan-500 shadow-lg scale-110' : 'border-slate-300 hover:border-cyan-400'
                      }`}></div>

                      {/* Content Card */}
                      <div className="bg-slate-50 rounded-lg p-4 sm:p-5 lg:p-6 hover:shadow-lg transition-all duration-300 ml-2 sm:ml-4 lg:ml-6">
                        <div className={`${
                          selectedExperience === index ? 'ring-2 ring-cyan-500 shadow-xl bg-cyan-50' : ''
                        } rounded-lg p-0 transition-all duration-300`}>
                          ...
                          {/* Technologies - Show first 3 on mobile, all on larger screens */}
                          <div className="flex flex-wrap gap-1">
                            {exp.technologies.slice(0, window.innerWidth < 640 ? 3 : exp.technologies.length).map((tech) => (
                              <span
                                key={tech}
                            ...
                            {exp.technologies.length > 3 && window.innerWidth < 640 && (
